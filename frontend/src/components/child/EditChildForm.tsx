...
interface EditChildFormProps {
  ...
  onSubmit: (childData: Partial<Omit<Child, 'id' | 'createdAt' | 'lastModifiedAt' | 'userId'>>) => void;
  onCancel: () => void;
}

export const EditChildForm = ({ initialChildData, onSubmit, onCancel }: EditChildFormProps) => {
  const [name, setName] = useState(initialChildData.name);
  const [birthdayOrAge, setBirthdayOrAge] = useState(initialChildData.birthday_or_age);
  const [photoUrl, setPhotoUrl] = useState(initialChildData.photo_url || "");
  const [interests, setInterests] = useState<string[]>(initialChildData.interests || []);
  const [currentInterest, setCurrentInterest] = useState("");
  const [errors, setErrors] = useState<{ [key: string]: string }>({});

  useEffect(() => {
    setName(initialChildData.name);
    setBirthdayOrAge(initialChildData.birthday_or_age);
    setPhotoUrl(initialChildData.photo_url || "");
    setInterests(initialChildData.interests || []);
  }, [initialChildData]);
